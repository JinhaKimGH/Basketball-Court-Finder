...
import { BackendResponse, BasketballCourt } from '../interfaces';
...
export default function Search(
    ...
    const [basketballCourts, setBasketballCourts] = React.useState<Array<BasketballCourt>>([]); // Array state of the basketball courts in the area
    ...
    function sortCourts(response : BackendResponse, latitude: number, longitude : number){
        return response.data.elements.sort((court1, court2) => {
            const distance1 = calcDistance(latitude, longitude, court1.lat, court1.lon);
            const distance2 = calcDistance(latitude, longitude, court2.lat, court2.lon);
    ...
    function handleArrow(event :  FormEvent<HTMLButtonElement>){
    ...
    return(
        <div>
            ...
            {(basketballCourts.length > 0 && isFound) ? <LeafletMap value={basketballCourts} index={Number(basketballCourts[index].id)} handleClick={setIndex}/> : ""}
        
            {(isFound && basketballCourts.length > 0) ? <div className="view-court">
                <button className="arrows" name="left" onClick={handleArrow}><i className="arrow left"></i></button>
                {basketballCourts.length > 0 && <Placecard key={basketballCourts[index].id} 
                                                    value={basketballCourts[index]} 
                                                    distance={calcDistance(coordinates.lat, coordinates.lon, basketballCourts[index].lat, basketballCourts[index].lon)}
                                                    username={props.username}/>}
                <button className="arrows" name="right" onClick={handleArrow}><i className="arrow right" ></i></button>
            </div> : ""}
